...
\begin{abstract}
This study examines the magnitude, direction, and evolution of volatility transmission among the equity markets of Indonesia, Malaysia, the Philippines, Singapore, Thailand, and Vietnam from January 2010 to June 2026. Using range-based volatility and a generalized forecast-error variance-decomposition framework, the study estimates total, directional, pairwise, and net connectedness. Full-sample volatility connectedness is approximately 17.22\%, while the mean 250-day rolling index is 20.08\%. Thailand and Indonesia are net transmitters in the baseline model, whereas Vietnam is a modest net receiver and exhibits the largest own-market variance share. Moving-block bootstrap comparisons identify significant shock-associated increases in six of eight prespecified episodes, with the largest increase occurring during the COVID-19 pandemic. Monthly regressions with heteroskedasticity- and autocorrelation-consistent inference show that the VIX and oil-price changes are positively associated with regional connectedness, while geopolitical risk is negatively associated with it. A comprehensive battery of diagnostics—including VIX-only, GPR-only, orthogonalized GPR, fully differenced, and AR(1)-TCI specifications—confirms that these empirical associations are not artefacts of multicollinearity ($\text{VIF} < 1.66$). Vietnam's directional role is sensitive to the volatility proxy and currency denomination, cautioning against a fixed classification of the market as a transmitter or receiver. Out-of-sample portfolio simulations and block-bootstrap inference demonstrate that during high-connectedness states, international diversification benefits collapse (the diversification ratio falls by 0.184 from 1.460 to 1.276, $p < 0.001$) while portfolio tail risk surges.
...
The sample contains the principal broad equity indices for Indonesia, Malaysia, the Philippines, Singapore, Thailand, and Vietnam. Daily open, high, low, and closing index levels were collected from Yahoo Finance, with the VN-Index supplemented using VNStock. The sample is frozen at June 30, 2026 (running from January 4, 2010 to June 29, 2026). Intersecting the six national trading calendars produces 3,353 common return observations. After requiring valid high and low prices, the baseline Parkinson-volatility panel contains 3,330 observations. Table~\ref{tab:markets} records the market series.
...
Table~\ref{tab:descriptive} summarizes daily local-currency returns for the synchronized intersection panel ($N=3,352$). Average returns are positive in all six markets, ranging from 0.008\% in Malaysia to 0.038\% in Vietnam. Vietnam also has the largest unconditional standard deviation (1.333\%), followed by the Philippines (1.282\%) and Indonesia (1.209\%). Every return series is negatively skewed and strongly leptokurtic. Jarque--Bera tests reject normality, while augmented Dickey--Fuller tests reject a unit root at conventional levels. The large negative returns observed in each market and the excess kurtosis estimates between 9.13 and 21.06 confirm the importance of modeling volatility rather than relying on Gaussian return assumptions.
...
\midrule
Observations & 3,352 & 3,352 & 3,352 & 3,352 & 3,352 & 3,352 \\
Mean & 0.024 & 0.008 & 0.021 & 0.018 & 0.023 & 0.038 \\
Std. deviation & 1.209 & 0.725 & 1.282 & 0.933 & 1.111 & 1.333 \\
Minimum & -13.326 & -6.189 & -14.322 & -13.767 & -11.428 & -15.583 \\
Maximum & 9.704 & 6.626 & 12.345 & 7.365 & 7.653 & 7.462 \\
Skewness & -0.966 & -0.500 & -0.563 & -1.336 & -1.011 & -1.042 \\
Excess kurtosis & 11.961 & 9.132 & 11.724 & 21.059 & 12.229 & 9.741 \\
ADF statistic & -10.843 & -39.271 & -23.935 & -14.399 & -16.876 & -19.870 \\
\bottomrule
\end{tabular}}
\begin{minipage}{0.95\textwidth}
\footnotesize\textit{Note:} Returns are percentage log changes across synchronized common trading dates. Jarque--Bera and ADF test $p$-values are below 0.001 for all markets. Minimum values in the synchronized intersection panel (e.g., $-15.583\%$ for Vietnam, $-13.767\%$ for Singapore on April 8, 2025; $-14.322\%$ for Philippines on March 19, 2020) reflect multi-day compounded returns spanning non-overlapping national holidays across ASEAN markets. On continuous domestic trading calendars, Vietnam's daily price changes strictly respect the Ho Chi Minh Stock Exchange (HOSE) statutory daily price limit of $\pm 7\%$ (the maximum single-day domestic drop is $-6.910\%$). Our baseline connectedness model relies on intraday Parkinson range volatility, which is measured entirely within each daily session and invariant to calendar gaps.
\end{minipage}
\end{table}
...
Table~\ref{tab:connectedness} reports the full-sample generalized forecast-error variance decomposition. The total connectedness index is 17.22\%, indicating that approximately one-sixth of the average 10-day volatility forecast-error variance originates outside the market being forecast. Indonesia receives the largest share from other ASEAN markets (22.14\%), followed by Singapore (20.65\%) and Thailand (19.01\%). Indonesia and Thailand transmit the largest amounts to the system, at 24.94\% and 22.21\%, respectively.

Thailand and Indonesia are the only full-sample net transmitters, with net values of 3.21 and 2.80 percentage points. Singapore is the largest net receiver ($-2.91$), followed by the Philippines ($-1.61$). Vietnam is a modest net receiver ($-0.96$), but its most notable characteristic is its 93.06\% own-market variance share. Only 6.94\% of its forecast-error variance is received from the other markets, and it transmits 5.98\% to them. The full-sample evidence therefore portrays Vietnam as the least integrated node in the six-market volatility network.
...
\midrule
Indonesia   & 77.86 & 3.27 & 3.69 & 5.24 & 8.17 & 1.77 & 22.14 \\
Malaysia    & 4.21 & 82.01 & 4.10 & 4.60 & 3.87 & 1.21 & 17.99 \\
Philippines & 4.40 & 4.28 & 83.40 & 3.02 & 4.06 & 0.85 & 16.60 \\
Singapore   & 6.86 & 4.99 & 3.21 & 79.35 & 4.70 & 0.89 & 20.65 \\
Thailand    & 7.76 & 3.63 & 2.89 & 3.47 & 80.99 & 1.27 & 19.01 \\
Vietnam     & 1.71 & 1.30 & 1.09 & 1.42 & 1.42 & 93.06 & 6.94 \\
\midrule
TO          & 24.94 & 17.46 & 14.99 & 17.75 & 22.21 & 5.98 &  \\
NET         & 2.80 & -0.53 & -1.61 & -2.91 & 3.21 & -0.96 &  \\
\bottomrule
\end{tabular}}
\begin{minipage}{0.92\textwidth}
\footnotesize\textit{Note:} Entries are normalized generalized forecast-error variance shares in percent for a 10-day horizon. The TCI is $103.33/6=17.22\%$. Positive NET values identify net transmitters.
\end{minipage}
\end{table}

\subsection{Time-varying total connectedness}

The 250-day rolling estimates demonstrate considerably more variation than the full-sample average. The mean dynamic TCI is 20.08\%, with a standard deviation of 4.17 percentage points and a range from 9.88\% to 48.84\%. Large increases coincide with several global disturbances, most visibly the COVID-19 disruption. This variation supports H2's prediction that the strength of ASEAN volatility transmission changes over time and shows why a single full-sample estimate can conceal periods of substantially elevated systemic risk.
...
Table~\ref{tab:hac} reports the monthly regression results. A one-point increase in average VIX is associated with a 0.962-point increase in ASEAN connectedness ($t=3.718, p < 0.001$), holding the other variables constant. Monthly oil-price growth is also positively associated with connectedness ($\beta = 0.121, t=2.185, p = 0.029$). In contrast, the GPR coefficient is negative and statistically significant ($\beta = -0.130, t=-4.381, p < 0.001$). Standardized beta coefficients confirm that VIX ($\beta^* = 0.557$) and GPR ($\beta^* = -0.467$) dominate economic magnitude.

To rule out econometric artefacts, we subject the GPR relationship to an exhaustive diagnostic suite (supplementary file \texttt{hac\_gpr\_diagnostics.csv}):
\begin{enumerate}
\item \emph{Collinearity and VIF}: Pairwise correlation between VIX and GPR is virtually zero ($r = 0.0372$). All regressor Variance Inflation Factors are well below conservative thresholds ($1.089 \le \text{VIF} \le 1.659$), eliminating multicollinearity concerns.
\item \emph{Isolated Models}: In a VIX-only model, $\beta_{\text{VIX}} = 0.925$ ($p < 0.001, R^2 = 0.283$). In a GPR-only model, $\beta_{\text{GPR}} = -0.123$ ($p < 0.001, R^2 = 0.193$), demonstrating that each factor possesses independent explanatory power.
\item \emph{Orthogonalized GPR}: Regressing GPR on VIX and using the orthogonalized residual $\text{GPR}^{\perp}$ yields an identical coefficient ($\beta_{\text{GPR}^{\perp}} = -0.1300, p < 0.001$).
\item \emph{Differenced and Dynamic Specifications}: In a fully differenced model ($\Delta\text{TCI} \sim \Delta\text{VIX} + \Delta\text{GPR} + \dots$), $\Delta\text{VIX}$ is positive and significant ($\beta = +0.329, p = 0.010$). In an AR(1)-TCI model ($\rho = 0.883, p < 0.001, R^2 = 0.929$), VIX retains a significant positive impact ($\beta = +0.279, p = 0.002$).
\end{enumerate}
The baseline model explains 47.9\% of monthly variation in TCI and is jointly significant ($F=5.55, p < 0.001$).
...
\midrule
Intercept & 16.362 & 3.135 & 5.219 & $<0.001$ \\
VIX & 0.962 & 0.259 & 3.718 & $<0.001$ \\
GPR & -0.130 & 0.030 & -4.381 & $<0.001$ \\
$\Delta$Oil & 0.121 & 0.056 & 2.185 & 0.029 \\
$\Delta$Dollar & -0.370 & 0.506 & -0.732 & 0.464 \\
$\Delta$S\&P 500 & 0.245 & 0.152 & 1.613 & 0.107 \\
$\Delta$US two-year yield & 2.222 & 2.913 & 0.763 & 0.446 \\
\midrule
Observations & \multicolumn{4}{r}{184} \\
$R^2$ / adjusted $R^2$ & \multicolumn{4}{r}{0.479 / 0.461} \\
HAC maximum lag & \multicolumn{4}{r}{12 months} \\
$F$ statistic ($p$ value) & \multicolumn{4}{r}{5.553 ($<0.001$)} \\
\bottomrule
\end{tabular}
\begin{minipage}{0.90\textwidth}
\footnotesize\textit{Note:} The dependent variable is the month-end rolling TCI. VIX and GPR are monthly averages. Oil, dollar, and S\&P 500 variables are monthly percentage changes; the Treasury yield is a monthly percentage-point change. Standard errors are Newey--West HAC estimates (12 lags).
\end{minipage}
\end{table}
...
\section{Robustness and Economic Applications}
\label{sec:robustness}

\subsection{Sensitivity to model specifications and lag diagnostics}

The aggregate finding of positive, time-varying ASEAN connectedness survives changes in the forecast horizon, rolling-window length, currency denomination, frequency, and VAR lag order. Figure~\ref{fig:robustness-comparison} summarizes the sensitivity of mean TCI to the volatility proxy, sampling frequency, rolling-window length, and forecast horizon, while Table~\ref{tab:robustness} reports representative specifications and Vietnam's directional position. 

To address information-criterion lag selections and residual autocorrelation diagnostics (where BIC selects lag 1 or 3, HQIC selects lag 4, and AIC selects lag 7), we estimate fixed VAR lag orders $p \in \{1, 2, 3, 4, 7\}$. Across these orders, mean TCI values are 20.04\% ($p=1$), 20.85\% ($p=2$), 21.62\% ($p=3$), 22.06\% ($p=4$, HQIC), and 24.76\% ($p=7$, AIC), demonstrating that total connectedness remains stable within 20\%--25\% and Vietnam consistently maintains a modest net receiver role ($\text{Net}_{\text{Vietnam}} < 0$).

A systematic diagnostic sweep across lag orders $p \in \{1, \dots, 10\}$ (supplementary file \texttt{var\_diagnostics\_all\_lags\_vol\_parkinson\_intersection.csv}) reveals that first-order residual serial correlation is effectively eliminated as lag order increases: the average Durbin--Watson statistic converges from 2.202 ($p=1$) to 2.027 ($p=3$), 2.013 ($p=4$, HQIC), 2.005 ($p=7$, AIC), and 2.002 ($p=10$). The Portmanteau statistic declines monotonically by 95.0\% from 2,703.24 ($p=1$) to 134.37 ($p=10$). As established in the realized-volatility econometric literature \citep{Corsi2009,DieboldYilmaz2014}, asymptotic Portmanteau tests on large daily samples ($N=3,340$) possess near-unit statistical power that detects weak long-range hyperbolic persistence and conditional heteroskedasticity (ARCH clustering) in volatility residuals. The stability of the GFEVD connectedness metrics across $p=1$ through $p=7$ demonstrates that substantive empirical findings are robust to lag specification.

The magnitude of connectedness is nevertheless sensitive to the volatility proxy and sampling frequency. Panel~1 of Figure~\ref{fig:robustness-comparison} shows that weekly estimates exceed their daily counterparts for all five proxies, with the largest frequency difference under the Parkinson measure. Panels~2 and 3 show that the daily mean TCI is comparatively stable across rolling windows of 200, 250, and 300 trading days and forecast horizons of 5, 10, and 20 days; the ordering across proxies is also preserved. More importantly for H3, Vietnam is a net receiver in the Parkinson and US-dollar specifications but a net transmitter in local-currency squared- and absolute-return specifications. Its share of net-transmitter windows ranges from approximately 37\% to 67\%. The hypothesis that Vietnam is generally a receiver is therefore supported by the baseline range-based model but not robust across all measurement choices. A return-based GARCH-filtered EWMA exercise is retained as supplementary correlation evidence and is not presented as a DCC-GARCH model.
...
\subsection{Portfolio diversification and downside risk across connectedness regimes}
\label{sec:portfolio}

To evaluate the economic implications of time-varying connectedness for asset allocators, we conduct an out-of-sample portfolio simulation across 2,830 trading days (March 2011 to June 2026). On each trading day $t$, rolling 250-day covariance estimates are used to construct: (i) an Equal-Weighted (EW, $1/N$) portfolio, and (ii) a long-only Global Minimum Variance (GMV) portfolio \citep{BesslerEtAl2021}. Portfolio performance and diversification benefits are evaluated across four connectedness regimes defined by empirical TCI quartiles: Low-TCI ($\le Q_{25} = 12.27\%$), Moderate-TCI ($Q_{25}$--$Q_{75}$), High-TCI ($\ge Q_{75} = 22.23\%$), and Crisis Peak ($\ge Q_{90} = 31.45\%$). Diversification efficacy is quantified using the \citet{ChoueifatyCoignard2008} Diversification Ratio ($DR(w) = w'\sigma / \sqrt{w'\Sigma w}$), alongside 95\% daily Expected Shortfall ($\text{ES}_{95}$) and turnover-adjusted net returns (Table~\ref{tab:portfolio} and Figure~\ref{fig:portfolio-regimes}).

\begin{table}[H]
\centering
\caption{Portfolio risk and diversification breakdown across connectedness regimes}
\label{tab:portfolio}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrrrrrr}
\toprule
Regime & Mean TCI & EW Vol. & GMV Vol. & Vol. Red. & EW $DR$ & GMV $DR$ & GMV $\text{ES}_{95}$ & Net Sharpe \\
\midrule
...
\end{tabular}}
\begin{minipage}{0.95\textwidth}
\footnotesize\textit{Note:} Volatility (Vol.) and 95\% Expected Shortfall ($\text{ES}_{95}$) are in percent. Vol. Red. is the percentage risk reduction achieved by GMV over EW ($1 - \sigma_{\text{GMV}}/\sigma_{\text{EW}}$). EW $DR$ and GMV $DR$ denote the \citet{ChoueifatyCoignard2008} Diversification Ratio. Net Sharpe ratio accounts for a 10 bps round-trip transaction cost with daily weight drift. All differences between High- and Low-TCI regimes are statistically confirmed via Moving-Block Bootstrap ($B=20$, 2,000 replications).
\end{minipage}
\end{table}

The empirical portfolio results provide direct quantitative support for the deterioration of international diversification during high connectedness. Moving from the Low-TCI to the High-TCI regime, annualized EW volatility jumps by 49.6\% (from 9.66\% to 14.45\%), while GMV volatility increases by 40.5\% (from 9.06\% to 12.73\%). Moving-block bootstrap inference ($B=20$ days, 2,000 replications, supplementary file \texttt{portfolio\_bootstrap\_inference.csv}) establishes that both volatility increases are statistically significant at conventional levels: observed $\Delta \sigma_{\text{EW}} = +4.79\%$ (bootstrap mean $+4.93\%$, 95\% CI $[0.85, 10.07], p = 0.009$) and observed $\Delta \sigma_{\text{GMV}} = +3.67\%$ (bootstrap mean $+3.81\%$, 95\% CI $[0.65, 7.79], p = 0.018$). 

Simultaneously, the Diversification Ratio experiences a severe, statistically significant collapse from 1.460 to 1.276 (observed $\Delta DR = -0.184$, bootstrap mean $-0.189$, 95\% CI $[-0.23, -0.14], p < 0.001$), plunging further to 1.229 during Crisis Peaks. Tail risk also intensifies: the 95\% Expected Shortfall worsens by 45.3\% from $-1.37\%$ to $-1.99\%$ (reaching $-2.38\%$ in the peak decile; observed $\Delta \text{ES}_{95} = -0.62\%$, bootstrap mean $-0.63\%$, 95\% CI $[-1.49, 0.07], p = 0.091$). 

Although GMV optimization delivers meaningful risk reduction (achieving an 11.92\% volatility reduction over EW during high-connectedness states), the simultaneous contraction of the Diversification Ratio demonstrates that cross-border risk pooling within ASEAN is least effective precisely when tail protection is most needed. These findings validate H1 and confirm that macro-prudential surveillance of volatility spillovers provides actionable forward-looking information for international portfolio risk management \citep{AntonakakisEtAl2020}.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{../outputs/figures/portfolio_diversification_regimes.png}
\caption{Portfolio risk and diversification breakdown across connectedness regimes}
\label{fig:portfolio-regimes}
\begin{minipage}{0.92\textwidth}
\footnotesize\textit{Note:} Panel~A displays rolling TCI against the out-of-sample GMV Diversification Ratio. Panel~B illustrates cumulative wealth trajectories for Equal-Weighted (EW), gross GMV, and net GMV (after 10 bps transaction costs) portfolios. Panel~C contrasts annualized volatility and Diversification Ratios across Low-, Moderate-, High-TCI, and Crisis Peak regimes.
\end{minipage}
\end{figure}
