...
The aggregate finding of positive, time-varying ASEAN connectedness survives changes in the forecast horizon, rolling-window length, currency denomination, frequency, and VAR lag order. Figure~\ref{fig:robustness-comparison} summarizes the sensitivity of mean TCI to the volatility proxy, sampling frequency, rolling-window length, and forecast horizon, while Table~\ref{tab:robustness} reports representative specifications and Vietnam's directional position. Within the daily Parkinson models, mean TCI remains close to 20\% and Vietnam remains a net receiver. Fixed VAR lag orders of one, two, and three produce mean TCI values of 20.04\%, 20.85\%, and 21.62\%, respectively.
