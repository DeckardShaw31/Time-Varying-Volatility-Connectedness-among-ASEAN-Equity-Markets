...
To evaluate the economic implications of time-varying connectedness for asset allocators, we conduct an out-of-sample portfolio simulation across 2,844 trading days (March 2011 to July 2026). On each trading day $t$, rolling 250-day covariance estimates are used to construct: (i) an Equal-Weighted (EW, $1/N$) portfolio, and (ii) a long-only Global Minimum Variance (GMV) portfolio \citep{BesslerEtAl2021}. Portfolio performance and diversification benefits are evaluated across four connectedness regimes defined by empirical TCI quartiles: Low-TCI ($\le Q_{25} = 12.25\%$), Moderate-TCI ($Q_{25}$--$Q_{75}$), High-TCI ($\ge Q_{75} = 22.22\%$), and Crisis Peak ($\ge Q_{90} = 31.44\%$). Diversification efficacy is quantified using the \citet{ChoueifatyCoignard2008} Diversification Ratio ($DR(w) = w'\sigma / \sqrt{w'\Sigma w}$), alongside 95\% daily Expected Shortfall ($\text{ES}_{95}$) and turnover-adjusted net returns (Table~\ref{tab:portfolio} and Figure~\ref{fig:portfolio-regimes}).
